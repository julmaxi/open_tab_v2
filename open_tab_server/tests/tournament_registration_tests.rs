...
use open_tab_server::{tournament::{CreateTournamentRequest, CreateTournamentResponse}, auth::{GetTokenRequest, CreateUserRequest, CreateUserResponse, RegisterUserResponse}};
...
async fn test_can_register_paricipant_via_key() {
    ...
    let response = fixture.post_json_no_body(&format!("/api/register/{}", encoded_key)).await;
    assert_eq!(response.status(), 200);
}
...
async fn test_registering_twice_returns_same_user() {
    ...
    let mut response = fixture.post_json_no_body(&format!("/api/register/{}", encoded_key)).await;
    assert_eq!(response.status(), 200);
