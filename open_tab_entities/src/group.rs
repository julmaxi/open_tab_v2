...
pub trait EntityGroupTrait {
    ...
    async fn get_many_with_type<C>(transaction: &C, type_name: Self::TypeId, uuids: Vec<Uuid>) -> Result<Vec<Entity>, Box<dyn Error>> where C: sea_orm::ConnectionTrait;
    async fn try_get_many_with_type<C>(transaction: &C, type_name: Self::TypeId, uuids: Vec<Uuid>) -> Result<Vec<Option<Entity>>, Box<dyn Error>> where C: sea_orm::ConnectionTrait;
...
pub async fn get_changed_entities_from_log<C>(transaction: &C, log_entries: Vec<crate::schema::tournament_log::Model>) -> Result<Vec<VersionedEntity<Entity, EntityType>>, Box<dyn Error>> where C: ConnectionTrait {
