...
impl DisplayBallot {
    async fn from_id<C>(ballot_id: Uuid, db: &C) -> Result<Self, anyhow::Error> where C: sea_orm::ConnectionTrait {
...
pub async fn check_is_authorized_for_debate_result_submission<C>(
    ...
    debate_id: Uuid,
) -> Result<bool, anyhow::Error> where C: sea_orm::ConnectionTrait {
