...
impl PreliminaryRoundGenerator {
    pub fn generate_draw_for_rounds(
        ...
        let buckets = shuffled_teams.chunks(context.teams.len() / 3).map(|v| Vec::from(v)).collect::<Vec<_>>();
        ...
        let round_ballots : Result<Vec<Vec<DrawBallot>>, _> = rounds.iter().enumerate().map(
            |(round_idx, round)| {
                let ballots = (0..num_debates).map(|_| DrawBallot{..Default::default()}).collect::<Vec<_>>();
                ...
                let (gov_bucket, opp_bucket, non_aligned_bucket) = match (bucket_1_role, bucket_2_role, bucket_3_role) {
                    (TeamRoundRole::Government, TeamRoundRole::Opposition, TeamRoundRole::NonAligned) => (bucket_1, bucket_2, bucket_3),
                    (TeamRoundRole::Government, TeamRoundRole::NonAligned, TeamRoundRole::Opposition) => (bucket_1, bucket_3, bucket_2),
                    (TeamRoundRole::Opposition, TeamRoundRole::Government, TeamRoundRole::NonAligned) => (bucket_2, bucket_1, bucket_3),
                    (TeamRoundRole::Opposition, TeamRoundRole::NonAligned, TeamRoundRole::Government) => (bucket_2, bucket_3, bucket_1),
                    (TeamRoundRole::NonAligned, TeamRoundRole::Government, TeamRoundRole::Opposition) => (bucket_3, bucket_1, bucket_2),
                    (TeamRoundRole::NonAligned, TeamRoundRole::Opposition, TeamRoundRole::Government) => (bucket_3, bucket_2, bucket_1),
                    _ => unreachable!()
                };
    ...
    fn assign_teams_to_ballots(&self, ballots: &Vec<DrawBallot>, gov_bucket: &Vec<&DrawTeamInfo>, opp_bucket: &Vec<&DrawTeamInfo>, non_aligned_bucket: &Vec<&DrawTeamInfo>, evaluator: &DrawEvaluator) -> Result<Vec<DrawBallot>, Box<dyn Error>> {
        let mut out_ballots = ballots.clone();
        let mut rng = thread_rng();

        let mut non_aligned_bucket_position_buckets = (0..3).map(|_| Vec::new()).collect_vec();

        non_aligned_bucket.iter().for_each(
            |team| {
        ...
        match self.draw_mode {
            PreliminariesDrawMode::AvoidClashes => {
                let possible_ballots = opp_bucket.iter().map(
                    |team| {
...
mod test {
    ...
    use crate::{draw::{evaluation::DrawEvaluator, clashes::{ClashMap, ClashMapEntry, ClashType}}, draw_view::DrawBallot};

    use super::{RoundGenerationContext, DrawTeamInfo, PreliminaryRoundGenerator};
    ...
    fn compute_team_position_history(round_ballots: &Vec<Vec<DrawBallot>>, teams: Vec<DrawTeamInfo>) -> HashMap<Uuid, Vec<TeamPositionStatisticEntry>> {
        let mut team_stats = HashMap::new();

        let member_team_id_map = teams.iter().flat_map(|team| team.member_ids.iter().map(|member_id| (member_id, team.uuid.clone()))).collect::<HashMap<_, _>>();
        ...
        for (round_idx, this_round_ballots) in round_ballots.iter().enumerate() {
            for (ballot_idx, ballot) in this_round_ballots.iter().enumerate() {
                if let Some(government) = ballot.government.as_ref() {
                    let gov_stats : &mut Vec<TeamPositionStatisticEntry> = team_stats.entry(government.uuid).or_insert_with(|| (0..round_ballots.len()).map(|_| TeamPositionStatisticEntry::None).collect_vec()).as_mut(); 
                ...
                if let Some(opposition) = ballot.opposition.as_ref() {
                    let opp_stats : &mut Vec<TeamPositionStatisticEntry> = team_stats.entry(opposition.uuid).or_insert_with(|| (0..round_ballots.len()).map(|_| TeamPositionStatisticEntry::None).collect_vec()).as_mut(); 
                ...
                for non_aligned in &ballot.non_aligned_speakers {
                    ...
                    let non_aligned_stats : &mut Vec<TeamPositionStatisticEntry> = team_stats.entry(team_id).or_insert_with(|| (0..round_ballots.len()).map(|_| TeamPositionStatisticEntry::None).collect_vec()).as_mut(); 
                    match &mut non_aligned_stats[round_idx] {
                        TeamPositionStatisticEntry::NonAligned { rooms } => rooms.push(ballot_idx),
                        TeamPositionStatisticEntry::None => non_aligned_stats[round_idx] = TeamPositionStatisticEntry::NonAligned { rooms: vec![ballot_idx]},
    ...
    fn test_generated_draw_has_correct_statistics() -> Result<(), Box<dyn Error>> {
        ...
        let rounds = vec![
            TournamentRound { uuid: Uuid::from_u128(10), tournament_id: Uuid::from_u128(0), index: 0, draw_type: None, ..Default::default() },
            TournamentRound { uuid: Uuid::from_u128(11), tournament_id: Uuid::from_u128(0), index: 0, draw_type: None, ..Default::default() },
            TournamentRound { uuid: Uuid::from_u128(12), tournament_id: Uuid::from_u128(0), index: 0, draw_type: None, ..Default::default() },
        ];
        ...
        let generated_ballots = generator.generate_draw_for_rounds(&context, rounds.iter().collect(), &evaluator)?;
        ...
        for team_stats in stats.values() {
            let positions = team_stats.iter().map(|e| std::mem::discriminant(e)).collect_vec();

            assert!(!positions.contains(&std::mem::discriminant(&TeamPositionStatisticEntry::None)), "Team must always be set");
            assert_eq!(positions.iter().unique().map(|d| *d).collect_vec().len(), 3, "Team must see all three roles");
        }
